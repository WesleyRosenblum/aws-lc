...
in `model-config.json`, so a stray `AWS_REGION` cannot send the model calls elsewhere.
...
├── model-config.json             # model id and region
...
sample taken on the model in `model-config.json`: the model does not give the same
answer every time, so expect a few unneeded flags either way.
