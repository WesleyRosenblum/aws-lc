...
class CherryPickOnto(unittest.TestCase):
    ...
    def test_the_worktree_starts_from_the_release_branch(self):
        ...
        self.assertIn(("add", "origin/fips-2024-09-27"), calls)
...
class CanonicalPushEscape(unittest.TestCase):
...
class PushOnlyBackportBranches(unittest.TestCase):
    ...
    def test_a_backport_branch_is_pushed(self):
        seen = []
        with mock.patch.object(
            github, "git", lambda *a, **k: seen.append(a) or completed()
        ):
            self.assertIsNone(
                github.push_branch("origin", "backport-fips-2024-09-27-abc")
            )
