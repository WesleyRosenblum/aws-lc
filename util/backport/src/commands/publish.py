...
def branch_state(release: str, local_branch: str) -> str:
    ...
    if worktree.exists() and cherry_pick_in_progress(worktree):
        return UNFINISHED
    if commits_ahead(f"origin/{release}", local_branch) < 1:
        return MISSING
    return OPENED
...
def report(
    run: Dict, outcomes: List[Tuple[str, str, str]], source_pr: Optional[str], repo: str
) -> None:
...
def cmd_publish(args) -> int:
    ...
    report(run, outcomes, args.pr, repo)
...
def offer_publish(run: Dict, branches: List[str], remote: str) -> None:
    ...
    if not ask_yn(f"Open pull requests for {len(branches)} branch(es)?"):
        ...
        return
    outcomes = run_publish(run, branches, remote, push_slug, None, False)
